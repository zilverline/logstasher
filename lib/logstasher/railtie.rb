  ...
  class Railtie < Rails::Railtie
    ...
    initializer :initialize_custom_logger, before: :initialize_logger, group: :all do
      ...
      if app_config.logstasher.enabled
        app_config.logstasher.logger_path ||= "#{Rails.root}/log/logstash_#{Rails.env}.log"
        ...
        app_config.logstasher.source = "#{Rails.application.class.parent_name.downcase}-#{Rails.env}"
      end
    end
